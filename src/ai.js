...
App.pathToNode = {}; // { 'std.app.nick.MyApp' : node }
App.namedPaths = {}
...
class Second {
    ...
    if(!nodesInMemory.length && nodesInMemory.length === 0){
      ...
      // - only ever run once 
      let firstResponse;
      if(process.env.LAUNCH_VERSION == '1'){
      	console.error('--SKIPPING FIRST INPUT (setup)--');
      } else {
	      firstResponse = await this.runRequest({
    ...
    if(process.env.LAUNCH_VERSION == '1'){
			...
			// let startupResponse = await universe.execService('services.second.default.startup',{
	    ...
	    startupResponse = await this.runRequest({
    ...
    return new Promise((resolve, reject)=>{
      ...
      secondReady.then(async ()=>{
        ...
        let CodeNode = App.namedPaths[startupPath];
...
const ThreadedSafeRun = (evalString, context = {}, requires = [], threadEventHandlers, requestId, mainIpcId, nodeId, timeout) => {
  return new Promise(async (resolve, reject)=>{
      ...
      universe: {
        ...
        execPath: (execPath, opts) => {
          return new Promise(async (resolve, reject)=>{
          	...
          	// execute runNodeCodeInVMInMemory for a path to a code node 
          	...
          	// TODO: validate that I'm allowed to run this codepath 
        		...
        		let execResult = await funcInSandbox.universe.runNodeCodeInVMInMemory(opts);
        ...
        // content-named/addressed storage 
        AppPathToNode: App.pathToNode,
        AppNamedPaths: App.namedPaths,
        ...
        getPathsAtPath: (prefixPath, opts)=>{
        	return new Promise((resolve,reject)=>{
	        	// list names at a path 
	        	// - levels dictate how far down to go 
	        	// - should also have a way of paginating results ("1-10 of 100")
	        	//   - could use ".__" as a reserved prefix? or ".pagination." 

						// returning all paths, even if only a placeholder 
						// - the ends-in-a-placeholder node paths get removed automatically 

	        	prefixPath = (prefixPath === undefined) ? '':prefixPath.toString();

	        	let levels = 0; // -1 = all

	        	let splitPrefixPath = prefixPath.split('.'); 

	        	let lastPart = splitPrefixPath[splitPrefixPath.length - 1];

	        	if(lastPart == '*'){
	        		levels = -1;
	        		splitPrefixPath.pop(); // remove last
	        		prefixPath = splitPrefixPath.join('.')
	        	} else if(lastPart.substring(0,1) == '*') {
						  let intMatch = lastPart.match(/\*\D{0}(\d+)$/); // "*2" where 0 is the default level (addition) 
						  levels = parseInt(intMatch ? intMatch[1]:0, 10) || 0;
	        		// TODO: error on invalid level input? (non-numbers?) 
	        		splitPrefixPath.pop(); // remove last
	        		prefixPath = splitPrefixPath.join('.')
	        	}

						if(prefixPath.substring(prefixPath.length - 1) != '.'){
	        		prefixPath = prefixPath + '.';
	        	}

	        	let prefixPathLevels = prefixPath.match(/\./g);
	        	prefixPathLevels = prefixPathLevels ? prefixPathLevels.length : 0;

	        	opts = opts || {};

						// only returning named objects, not placeholders! 
			      let foundPaths = Object.keys(App.namedPaths).filter(name=>{
			      	// if prefix matches, then it is the same path 
			        if(name.indexOf(prefixPath) === 0){
			        	// levels 
			        	let pathLevels = name.match(/\./g);
			        	pathLevels = pathLevels ? pathLevels.length : 0;
			        	if(levels === -1 || ((pathLevels - prefixPathLevels) <= levels)){
			        		return true;
			        	}
			        }
			      });
        ...
        getNodeAtPath: (path, opts)=>{
        	return new Promise((resolve,reject)=>{
        		...
        		let refNode = App.namedPaths[path];
        		let returnNode;
        		if(refNode){
        			if(opts.fields){
        ...
        getNodesForPathPattern: (patternArr, opts)=>{
        	return new Promise(async (resolve,reject)=>{
        		...
        		// TODO: opts.followMirror ? returns data for the mirror'd path 
        		let nodes = [];
        		for(let node of App.nodesDbParsed){
        			let nodePath = node._path;
        			let matchesAnyPattern;
        			for(let pattern of patternArr){
        				if(matchesAnyPattern){
        					continue;
        				}
	        			let isPathMatch = funcInSandbox.universe.pathMatch(nodePath, pattern);
	        			if(isPathMatch){
        ...
        putNodeAtPath: (path, key, node, opts)=>{
        	return new Promise(async (resolve,reject)=>{
        		...
        		let existingNode = App.pathToNode[path];
	            ...
	            node = {
	              ...
	              type: node.hasOwnProperty('type') ? node.type : undefined, 
	              data: node.hasOwnProperty('data') ? node.data : undefined, 

	              placeholder: false, // should no longer be a placeholder node
                ...
                command: 'updateNode', // whole thing for now
                node,
                skipWaitForResolution: null,
                skipRebuild: null
	            }, (r)=>{
			          console.log('Updated content-addressed node');
	              resolve(r.data);
	            })
        			...
        			for(let parentPathIdx in parentPaths){
        				...
        				let pNode = App.pathToNode[parentPath];
        				if(pNode){
        					parentNodes.push(pNode);
        				} else {
        					// doesn't exist, needs to be created as a placeholder 
        					console.log('Creating a placeholder node for path:', parentPath, 'addrName:', addrName);
        					let prevNode = parentNodes[parentPathIdx - 1];
			            let newlySavedNodeResponse = await setupIpcWatcher({
		                command: 'newNode', // whole thing for now
		                node: {
		                	nodeId: prevNode ? prevNode._id:null,
		                	name: addrName,
		                	placeholder: true,
		                	type: 'placeholder',
		                	data: null,
			            ...
			            console.log('Saved a new placeholder node');
			            parentNodes.push(newlySavedNodeResponse.data);
        				}
        			...
        			// save this new node 
        			console.log('Saving new path-addressed node:', path);
        			let prevNode2 = parentNodes[parentNodes.length - 1];
	            setupIpcWatcher({
                command: 'newNode', // whole thing for now
                node: {
                	nodeId: prevNode2 ? prevNode2._id:null,
                	name: parentPathSplit[parentPathSplit.length - 1],
                	placeholder: false,
                	type: node.type,
                	data: node.data,
                	active: true
	            ...
	            console.log('Saved a new content-addressed node');
	              resolve(r.data);
	            })
        ...
        execService: (serviceName, opts) => {
        	return new Promise(async (resolve, reject)=>{
        		...
        		let serviceCodeNodeResult = await funcInSandbox.universe.runNodeCodeInVMInMemory({
        	...
        	return new Promise(async (resolve, reject)=>{
        		...
        		//   - checking for matches 
	        		...
	        		if(permissionForActionPathNode){
	        			console.log('permissionForActionPathNode:', permissionForActionPathNode._path);
        		...
        		let breakCheck;
    				...
    				for(let permissionEntry of permissionsTreeArray){
    					...
    					if(!permissionCodeNode){
    						console.error('Missing permissionCodeNode for path:', permissionEntry.code, 'in permission:', permissionForActionPathNode._path);
    						continue;
    					}
	        		let permissionsOutputResult = await funcInSandbox.universe.runNodeCodeInVMInMemory({
        	...
        	return new Promise(async (resolve, reject)=>{
        		...
        		if(permissionsNode){
	        		switch(permissionsNode.type){
	        			...
	        			default:
	        				console.error('Invalid permissionsNode.type', permissionsNode.type, permissionsNode._path);
	        				break;
    ...
    try {
      ...
      .catch(err=>{
        console.error('---Failed in VM1!!!---- internal_server_error. --', codeNode ? codeNode._path:null, ob.nodeId, err, err.toString ? err.toString():null, output);
        resolve({
      ...
      console.error('---Failed in VM2!!!----', codeNode ? codeNode._path:null, ob.nodeId, err, err.toString ? err.toString():null);
      resolve({
          type: 'internal_server_error_public_output:0.0.1:local:3298ry2398h3f',
          data: {
            error: true,
